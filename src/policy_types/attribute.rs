...
impl Attribute {
    ...
    pub fn must_zpr_internal_attr<S: Into<String>, T: Into<String>>(
        name: S,
        value: T,
    ...
    /// ## Panics
    /// - if passed `name` does not start with `zpr`.
    pub fn must_zpr_internal_attr_mv<S: Into<String>, T: Into<String>>(
        name: S,
        value: T,
    ) -> Result<Self, AttributeError> {
        match name
...
mod test {
    ...
    fn test_attrributes_internal() {
        let a =
            Attribute::must_zpr_internal_attr("zpr.role", "admin").expect("zpr prefix not found");
    ...
    fn test_zplc_key_all_domains() {
        ...
        let zpr_attr =
            Attribute::must_zpr_internal_attr("zpr.test", "value").expect("zpr prefix not found");
        assert_eq!("zpr.test", zpr_attr.zplc_key());
    }

    #[test]
    fn test_attributes_internal_error() {
        let a1 = Attribute::must_zpr_internal_attr("zdp.role", "admin");
        assert!(a1.is_err());
        let a2 = Attribute::must_zpr_internal_attr("role", "admin");
        assert!(a2.is_err());
    }

    #[test]
    fn test_attributes_internal_error_mv() {
        let a1 = Attribute::must_zpr_internal_attr_mv("zdp.role", "admin");
        assert!(a1.is_err());
        let a2 = Attribute::must_zpr_internal_attr_mv("role", "admin");
        assert!(a2.is_err());
    }
}
